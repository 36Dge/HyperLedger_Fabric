...
import (
	"fabricProject/orderer/common/msgprocessor"
	"io"
	"time"
)
...
//handle reads requests from a broadcast stream ,process them .and returns the responses to the stream
func (bh *Handler) Handle(srv ab.AtomicBroadcast_BroadcastServer) error {
	addr := util.ExtractRemoveAddress(srv.Context())
	logger.Debugf("staring new broadcast loop for %s", addr)
	for {
		msg, err := srv.Recv()
		if err == io.EOF {
...
//processmessage validates and enques a single message
...
func ClassifyError(errerror) cb.Status {
