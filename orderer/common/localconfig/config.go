...
import (
	"encoding/json"
	"fabricProject/common/viperutil"
	"fmt"
	"path/filepath"
	"sync"
	"time"
...
//load parses the order YAML file and envirionment. producing
//a struct suitable for config use. retueing error on failure.
func Load() (*TopLevel, error) {
	return cache.load()
}

//cofigcache stores marshalled bytes of config structures that produced
//from enhancedexactunmarshal .cache key is the path of the configuration
//file that was used.
...
//load will load the configuration and cache it one the first call.
//subsequent calls will return a clone of the configuration that was previously
//loaded.
